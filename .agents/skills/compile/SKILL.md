...
**Before compiling**, read `wiki-exclusions.yml` and remove any listed bookmarks from the compile candidate set. Excluded bookmarks are intentional absences from the wiki graph, not backlog. Then apply the Quality Gate from `CLAUDE.md` → "Quality Gate (junk filter)" to any remaining bookmark not yet in a wiki page's `sources:` list. Quarantine failures to `_trash/` with a log entry. Skip quarantined files — never compile them into the wiki.
...
Read `wiki/INDEX.md`, `wiki-exclusions.yml`, and any existing wiki pages. Check the `sources` frontmatter field in each wiki page to identify which bookmarks have already been compiled. This enables incremental updates.
...
- Never reintroduce a bookmark listed in `wiki-exclusions.yml` unless the user explicitly asks to reinstate it
