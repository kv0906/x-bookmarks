...
**Also enforce the Quality Gate** from `CLAUDE.md` → "Quality Gate (junk filter)". Read `wiki-exclusions.yml` first: bookmarks listed there are intentional exclusions and must not be reported as uncompiled. Scan every remaining bookmark across all topic folders for reject criteria (broken content, zero-value, duplicates). Report any failures as a "Quality Gate Violations" section in the lint report and recommend quarantining them to `_trash/`. Do NOT auto-move — lint is read-only.
...
**Uncompiled bookmarks** — bookmark files not referenced in any wiki page's `sources` frontmatter and not listed in `wiki-exclusions.yml`. These are raw sources the wiki hasn't absorbed yet.

**Intentional exclusions** — bookmarks listed in `wiki-exclusions.yml`. Report them separately only if the manifest is stale, contradictory, or missing a reason.
...
- `folder/filename.md` — excluded with reason
...
- Do not count intentional exclusions as issues unless the exclusion manifest itself is wrong or stale.
